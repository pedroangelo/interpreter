module Gradual.ConstraintUnification (
    unifyConstraints
) where
...
unifyConstraints types [] _ = return (types, [])

-- for consistenty constraints
unifyConstraints types (Consistency t1 t2 : cs) counter
    -- U ((t ~C t) : cs) => U cs
    | t1 == t2 = unifyConstraints types cs counter
    -- U ((? ~C t) : cs) => U cs
    | t1 == DynType = unifyConstraints (types ++ [t2]) cs counter
    -- U ((t ~C ?) : cs) => U cs
    | t2 == DynType = unifyConstraints (types ++ [t1]) cs counter
    ...
    | isTupleType t1 && isTupleType t2 && compareSize t1 t2 = do
        ...
        let constraints = zipWith Consistency types1 types2
        unifyConstraints types (constraints ++ cs) counter
...
unifyConstraints types (Equality t1 t2 : cs) counter
    ...
    | isTupleType t1 && isTupleType t2 && compareSize t1 t2 = do
        let TupleType types1 = t1
        let TupleType types2 = t2
        let constraints = zipWith Equality types1 types2
        unifyConstraints types (constraints ++ cs) counter
    -- U (({l1i:T1i} =C {l2i:T2i}) : cs)
    -- => U ((T11 =C T21) : ... : (T1n =C T2n) : cs)
    | isRecordType t1 && isRecordType t2 && compareLabels t1 t2 = do
...
belongs (VarType var) typ
    ...
    | isMuType typ =
        let (Mu var' t') = typ
        in (var' /= var) && belongs (VarType var) t'
    | otherwise = False
belongs _ _ = False
